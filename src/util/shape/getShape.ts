...
const getShape = ({
    ...
    if (
        ...
        return null; // returns if svgRef does not point to svg element or tool is not a shape
    ...
    const params = getShapeParams({
        ...
        shape,
        options,
    }); // a function that returns the right parameter given the set of start point, end point and the type of shape
